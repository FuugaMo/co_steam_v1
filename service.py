...
class T2IService:
    def __init__(self, args):
        ...
        # ComfyUI input dir for reference images (if exists)
    ...
    async def generation_worker(self):
        ...
        while self.running:
            ...
            try:
                ...
                print(f"║ ✅ T2I_COMPLETE sent! path={result['image_path']}")
    ...
    def generate_image(self, request_id: str, image_keywords: list, request: dict) -> dict:
        ...
        prompt_data = build_prompt(
            ...
            detail_level=self.detail_level,         # Control Pad控制
        ...
        # 日志输出（显示结构化信息）
        print(f"║ Style: {structure['style']} | Detail: {structure['detail_level']} | Version: {self.version_tag}")
        ...
        if self.reference_images:
            print(f"║ Reference: {self.reference_images}")

        # 加载workflow模板（有参考图则自动切换到 sd15_ref）
        workflow_name = self.args.workflow
        if self.reference_images and workflow_name == "sd15_fast":
            workflow_name = "sd15_ref"
            print(f"║ Using reference workflow: {workflow_name}")
    ...
    def inject_reference_image(self, workflow: dict) -> dict:
        """Inject reference image into LoadImage node (first image only)"""
    ...
    async def run(self):
        ...
        self.detail_level = self.args.detail_level          # low/medium/high（教学抽象度）
        ...
        async def on_config(msg: Message):
            ...
            if msg_type == MessageType.CONFIG_UPDATE.value:
                ...
                if param == "style":
                    ...
                    print(f"T2I: Detail level → {value}")
        ...
        status_msg = Message.status(Source.T2I, "ready", {
            ...
            "workflow": self.args.workflow,
            "style": self.style,
            "detail_level": self.args.detail_level,
            "version_tag": self.version_tag
        })
        ...
        print(f"Style: {self.style} | Detail: {self.detail_level}")
        print(f"Version: {self.version_tag}")
...
def main():
    ...
    parser.add_argument('--style', default='Educational diagram, clear structure, pedagogical visualization',
                        help='Free-text style description')
    parser.add_argument('--detail-level', default='',
                        help='Detail level (optional; leave empty to disable)')
